    ...
    if (target === "swift") {
        ...
        if (nextToSources) {
            generatedFiles.forEach(({ sourcePath, fileName, content }) => {
                const dir = path.join(path.dirname(path.relative(rootPath, toPath(sourcePath))), outputPath);
        ...
        if (nextToSources ||
            ...
            generatedFiles.forEach(({ sourcePath, fileName, content }) => {
                let dir = outputPath;
                if (nextToSources) {
                    dir = path.join(path.dirname(path.relative(rootPath, toPath(sourcePath))), dir);
                    if (!localfs_1.fs.existsSync(dir)) {
                        localfs_1.fs.mkdirSync(dir);
                    }
                }
